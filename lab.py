...
def evaluate_classifier(trainer: Trainer, tokenized_test) -> dict:
    ...
    per_class_f1 = {id2label[i]: float(per_class_f1_arr[i]) for i in range(len(per_class_f1_arr))}
    per_class_precision = {id2label[i]: float(per_class_precision_arr[i]) for i in range(len(per_class_precision_arr))}
    per_class_recall = {id2label[i]: float(per_class_recall_arr[i]) for i in range(len(per_class_recall_arr))}
...
def main() -> None:
    ...
    # Add one prob_<label> column per class
    ...
    # Confusion matrix CSV (rows = true label, columns = predicted label)
    ...
    for label in label_names:
        print(
            f"  {label:10s}  F1={metrics['per_class_f1'][label]:.4f}"
            f"  P={metrics['per_class_precision'][label]:.4f}"
            f"  R={metrics['per_class_recall'][label]:.4f}"
        )
